                            ...
                            <select class="form-control select2-member" wire:model="form.member_id" id="member_id">
                                ...
                                @foreach($members as $member)
                                    <option value="{{ $member->id }}" 
                                            ...
                                            {{ $form->member_id == $member->id ? 'selected' : '' }}>
                                        {{ $member->name }} - {{ $member->nik }}
                                    </option>
                                @endforeach
                            </select>
                            @if($selectedMember)
                                <small class="text-muted">Selected: {{ $selectedMember->name }} ({{ $selectedMember->nik }})</small>
                            @endif
...
@push('js')
    <script>
        ...
        document.addEventListener('livewire:updated', function () {
            ...
            // Set the selected value for Select2 after re-initialization
            setTimeout(function() {
                if ($('#member_id').val()) {
                    $('#member_id').trigger('change');
                }
